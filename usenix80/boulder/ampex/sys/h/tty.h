...
struct tty
{
	...
	char	t_pagec;	/* char to start output for one page */
	char	t_nlcnt;	/* number of newlines since last input */
	int	t_state1;	/* extra state info */
	union {
		struct tc;
		struct clist t_ctlq;
	} t_un;
...
/*
 * internal state1 bits
 */
#define PAGEMODE 01		/* auto-stop every page */
#define BACKSPACE 02		/* in middle of erase (last char was erase) */
...
#define TIOCDRAIN	(('t'<<8)|19)	/* wait for dr-11k queue to drain */
