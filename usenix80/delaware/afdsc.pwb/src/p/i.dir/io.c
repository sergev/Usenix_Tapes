...
dump_line () {					/* dump_line is the routine that actually effects the printing of the
						   new source.  It prints the label section, followed by the code
						   section with the appropriate nesting level, followed by any comments 
						*/
register int    cur_col,
                target_col;

    bl_line = true;					/* if we don't find otherwise, assume a blank line */

    if (ind_level == 0)
	ind_stmt = 0;					/* this is a class A kludge. don't do additional statement
							   indentation if we are at bracket level 0 */

    if (e_lab != s_lab || e_code != s_code)
	++code_lines;					/* keep count of lines with code */

    if (e_lab != s_lab) {				/* print lab, if any */
	if (pcase)					/* if the label is really a case, we must indent */
	...
	cur_col = pad_output (cur_col, target_col);
							/* pad_output writes enough tabs and spaces to get the current
							   char position up to target_col */
	write (output, s_code, e_code - s_code);
	cur_col = count_spaces (cur_col, s_code);
	bl_line = false;				/* line not blank */
    }

    if ((cur_col - 1) > max_col)			/* check for line too long */
	printf ("%d: Code has %d chars, max is %d\n", line_no, (cur_col - 1), max_col);

    if (s_com != e_com) {				/* print comment, if any */
	if (cur_col > com_col) {			/* if comment can't start in proper column, put it on next line 
							*/
	    write (output, "\n", 1);
	    cur_col = 1;
	    ++out_lines;
	...
	if ((cur_col - 1) > max_col)			/* check for too long comment */
    ...
    *(e_com = s_com) = '\0';
    ...
    ++out_lines;
    decl_on_line = in_decl;				/* if we are in the middle of a declaration, remember that fact
							   for proper comment indentation */
    ind_stmt = in_stmt & ~in_decl;			/* next line should be indented if we have not completed this
...
register int    i;
	...
	bp_save = be_save = 0;
	if (buf_ptr < buf_end)
	    return;					/* only return if there is really something in this buffer */
    }
	...
	had_eof = true;
	*buf_end++ = ' ';
	*buf_end++ = '\n';				/* insert extra newline.  it will eventually get indent to stop 
							*/
    }
...
*/
int     pad_output (current, target)		/* writes tabs and blanks (if necessary) to get the current output
						   position up to the target column */
...
char    pobuf[256];				/* pad characters are stored here before writing */
    ...
    p = pobuf;
    while (curr < target) {
	if ((target - curr) >= tabsize) {
	    *p++ = '\t';				/* put a tab into buffer */
	    curr = ((curr - 1) & tabmask) + tabsize + 1;
	}
	else {
	    while (curr++ < target)
...
*/
int     count_spaces (current, buffer)		/* this routine figures out where the character position will be after
...
register char  *s2;				/* local pointer into second string */
    ...
    s1 = str1;
    s2 = str2;
    while (*s1) {					/* compare no further than end of first string */
