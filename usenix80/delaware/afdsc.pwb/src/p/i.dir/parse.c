...
int     p_stack[50] stmt;			/* this is the parser's stack */
...
int     i;
    ...
    printf ("%2d - %s\n", tk, token);
#endif
    while (p_stack[tos] == ifhead && tk != elselit) {   /* true if we have an if without an else */
	...
	case decl: 					/* scanned a declaration word */
	    search_brace = btype_2;			/* indicate that following brace should be on same line */
	    if (p_stack[tos] != decl) {			/* only put one declaration onto stack */
		break_comma = true;			/* while in declaration, newline should be forced after comma 
							*/
		p_stack[++tos] = decl;
		il[tos] = i_l_follow;

		if (ljust_decl) {			/* only do if we want left justified declarations */
		    ind_level = 0;
		    for (i = tos - 1; i > 0; --i)
			if (p_stack[i] == decl)
			    ++ind_level;		/* indentation is number of declaration levels deep we are */
	...
	case lbrace: 					/* scanned { */
	    ...
	    if (p_stack[tos] == stmt || p_stack[tos] == decl || p_stack[tos] == stmtl)
		++i_l_follow;				/* it is a random, isolated stmt group or a declaration */
	    else {
		if (s_code == e_code) {			/* only do this if there is nothing on the line */
		    --ind_level;			/* it is a group as part of a while, for, etc. */
		    if (p_stack[tos] == swstmt)
			--ind_level;			/* for a switch, brace should be two levels out from the code 
							*/
		}
	    }
	    ...
	    p_stack[++tos] = lbrace;
	    il[tos] = ind_level;
	    p_stack[++tos] = stmt;			/* allow null stmt between braces */
	    il[tos] = i_l_follow;
	    break;

	case whilestmt: 				/* scanned while (...) */
	    if (p_stack[tos] == dohead) {		/* it is matched with do stmt */
	...
	case elselit: 					/* scanned an else */
		...
		ind_level = il[tos];			/* indentation for else should be same as for if */
		i_l_follow = ind_level + 1;		/* everything following should be in 1 level */
		p_stack[tos] = elsehead;		/* remember if with else */
		search_brace = btype_2;
	    }
	...
	case swstmt: 					/* had switch (...) */
	    p_stack[++tos] = swstmt;
	    cstk[tos] = case_ind;			/* save current case indent level */
	    il[tos] = i_l_follow;
	    case_ind = i_l_follow + 1;			/* cases should be one level down from switch */
	    i_l_follow =+ 2;				/* statements should be two levels in */
	    search_brace = btype_2;
	    break;

	case semicolon: 				/* this indicates a simple stmt */
	    break_comma = false;			/* turn off flag to break after commas in a declaration */
	    p_stack[++tos] = stmt;
	    il[tos] = ind_level;
	    break;
...
register int    i;				/* local looping variable */
	...
	switch (p_stack[tos]) {
		...
		switch (p_stack[tos - 1]) {
		    ...
		    case stmt: 
		    case stmtl: 			/* stmtl stmt or stmt stmt */
			p_stack[--tos] = stmtl;
			break;
			...
			for (i = tos - 1;
				...
				--i);
			i_l_follow = il[i];		/* for the time being, we will assume that there is no else on
		...
		if (p_stack[tos - 1] == dohead) {       /* it is termination of a do while */
		    p_stack[--tos] = stmt;
		    break;
		}
		else
