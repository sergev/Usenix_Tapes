 ...
 *
 *	DFC	07/26/78	variables termhold and pitchold for
 *				-P and -T flags.
 *
 */
int debug 0;			/* JHU debug switch */
int bugmode 3;			/* options selected on JHU debugging */
...
int print 1;
char nextf[NS];
char searchf[NS] "/usr/lib/mac/tmac.xxxxx";
int nfi 18;
#ifdef NROFF
char termtab[NS] "/lib/mac/term/";
int tti 14;
char termhold[15] "37";
char pitchold[3] "10";
#endif
char suftab[] "/lib/mac/suftab";
int init 1;
int fc IMP;
int eschar '\\';
int pl 11*INCH;
...
int ldrch LEADER;
int xxx;
int batchflg 0;   /*  Batch Variables  */
char *endbname;
char batchf[43];   /*  ---------------  */
int ph, ph1;	/* .ph (set pitch) variables */
...
caseht(), casexf(), casexp(), caseph(), casebg(), casepr(), casetf(), casesh();
struct contab {
	int rq;
	int (*f)();
}contab[NM]{
