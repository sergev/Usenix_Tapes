...
extern int vflag;
extern int xxx;
extern char *endbname;  /*  Batch Variables  */
extern batchf[];
extern int batchflg;  /*  -----------  */
...
ptlead(){
	...
	while(k > 0){
		...
		oput(((~i) & 037) | 0140);
		if((paper > (31680)) && !papflg){		/* 11 in./page * 144 units/in. * 20 pages */
			prstr("Excessive paper use.\n");
			papflg++;
			if(ptid != 1){	/* if not going to standard output (i.e. -t flag) */
				lead = 0;
				done2(0200);
			}
		}
...
dostop(){
	...
	/*  Batch Code -- Initial lead on new file  */
	if(batchflg)  /*  && !gflag  */
		oput(0140);
	/*  ------------  */
}
