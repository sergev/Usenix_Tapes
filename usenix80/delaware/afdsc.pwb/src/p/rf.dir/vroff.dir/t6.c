...
extern int ch0;
extern int lg;
char fontfile[] "/lib/mac/font/XXXXX";
char msg[] "\n font mnt info bad";
int ffi 14;
extern int bd;
extern int level;
...
temporary fix to give space for unavailable char
*/
	else if(!(k = *(p + i) & BMASK)){i=0; goto g0;}
/* assuming that Wn (width table) is integer and the mask is in 00X000 */
	if(setwdf)v.ct =| ((k>>8) & 3);
g1:
	k = ldiv(0, (k&0377)*(xpts&077),20);
	if(ldivr >= 10)k++;
	if(cs = cstab[xfont]){
		if(ccs = ccstab[xfont])x = ccs; else x = xpts;
		cs = ldiv(0, (cs&0377)*(x&077), 20);
		if(ldivr >= 10)cs++;
	}
...
mot(){
	...
	if(i = atoin()){
		if (vflag) j = VERT;
		p= lmot( quant(i,j), cp= cbuf, vflag, NRULE);
		*p= 0;
	}
	getch();
	vflag = 0;
...
casefp(){
	...
	if (skip()) return;
	for (k= 0; (((j= getch())&CMASK) != '.') && k<3; k++) {
		if ((j= (fontfile[ffi+k] = j&BMASK) - '0') <0 || j>9) {
			prstr(msg); return;
		}
		set = set*10 + j;
	}
	if (j & CMASK != '.') {prstr(msg); return;}
	fontfile[ffi+k++] = '.';
	if ((sub = (fontfile[ffi+k++] = getch() & BMASK) -'0') <0 || sub >5) {
		prstr(msg); return;
	}
	fontfile[ffi+k] = '\0';
	...
	if(read(k,fontab[i],2*(256-32)) != 2*(256-32)){
	c1:
		prstr("Cannot read ");
		goto c0;
	}
	close(k);
	fontcode[i] = (set << 4)| sub;
...
casevs(){
	...
	skip();
	vflag++;
	dfact = 10; /*default scaling is points!*/
	res = VERT;
	i = inumb(&lss);
	if(nonumb)i = lss1;
	if(i < VERT) i = VERT;
