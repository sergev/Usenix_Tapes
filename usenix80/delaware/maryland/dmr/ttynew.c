 ...
 * Modified to include fancy backspacing, 8-bit transparent input
 * ctrl-'S'/ctrl'Q' stop/start of output
 * sgtty returns input queue status in u.u_arg[4] for ``empty'' system call
 * Last modification: June 18, 1979
 */
#include "../param.h"
#include "../systm.h"
#include "../user.h"
...
struct tty *atp;
{
	...
	if (c & (FRMERR | PARERR))      /* bad character */
		if (t_flags & RAW)
			c = 0;
		else
			return;         /* ignore */

	if (t_flags & (EVENP | ODDP))
		c =& 0177;              /* normal: 7 bits */
	else
		c =& 0377;              /* 8 bit transparent mode */
	...
	if (tp->t_rawq.c_cc>=(TTYHOG-1))
		c = BELL;       /* ring his chimes */
	else {
		putc(c, &tp->t_rawq);   /* pass it along */
		if (t_flags&RAW || c=='\n' || c==004) {
			wakeup(&tp->t_rawq);
			tp->t_delct++;
			}
	...
	if (t_flags&ECHO) {
		if (c == tp->t_erase && !(t_flags&RAW) && (t_flags&CRT)) {
			ttyoutput('\010', tp);  /* backspace */
			ttyoutput(' ',    tp);  /* erase */
			ttyoutput('\010', tp);  /* backspace */
			}
		else
...
struct tty *atp;
{
	...
	if ((addr->tttcsr&DONE)==0 || tp->t_state&(FROZEN|TIMEOUT))
		return;
...
int *atp, *av;
{
	...
	if(v = av) {
		*v++ = tp->t_speeds;
		...
		v->hibyte = tp->t_kill;
		v[1] = tp->t_flags;
		/* return the input queue status for the RAND empty call. */
