...
struct inode *aip;
{
	...
	while (dp = dread(ip, doff)) {
		...
		/* RAND addition - Maybe needs to be done elsewhere */
		if (dp->ino > nfiles)
		{       printf ("Preposterous i-number (%l)in inode %l\n",
				dp->ino,ino);
			nerror =| 0100;
			continue;
		}
...
pass2(aip)
{
	...
	if (ip->i_nlink==ecount[i] && ip->i_nlink!=0)
		return;
	if (headpr==0) {
		printf("inode   direct  inode\n");
		printf("number  entries link-cnt\n");
		headpr++;
	}
	printf("%l	%d	%d\n", ino,
	    ecount[i]&0377, ip->i_nlink&0377);
	if( (ecount[i]&0377) > (ip->i_nlink&0377) ) nerror =| 0100;
}
