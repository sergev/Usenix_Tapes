/* find -- find files in a pathname.
	...
	Bug Fixes:
	    Corrected to not ignore the first line in /etc/passwd
		(as suggested by Bob Kirby).
	    Corrected strange handling of 'fin' in routine 'getnum'.
	    Flushes input buffer after closing password file.
	    1/3/80 by Fred Blonder
...
extern int fin[];       /* used by getchar */
...
char str[20], *sp, c;
	...
	*fin = open("/etc/passwd",0);
	...
	RET:
	close(*fin);
	/* kludgily flush input buffer */
	*fin = -1;
	while(getchar());
	/* redirect getchar to std input */
	*fin = 0;
	return(i);
}
