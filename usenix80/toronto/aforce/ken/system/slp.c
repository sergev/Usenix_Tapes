...
loop:
	...
	for(rp = &proc[1]; rp < &proc[NPROC]; rp++) {
		if(rp->p_stat==SRUN && (rp->p_flag&SLOAD)==0) {
			a = rp->p_time&0377;
			if(rp->p_nice > NICEFENCE)	/* make her appear younger */
				a =/ (rp->p_nice - NICEFENCE + 1);
			if(a > n) {
				p1 = rp;
				n = a;
			}
	...
	n = -1;
	for(rp = &proc[1]; rp < &proc[NPROC]; rp++) {
		if((rp->p_flag&(SSYS|SLOCK|SLOAD|SGONE))==(SLOAD|SGONE) &&
		   (rp->p_stat==SRUN || rp->p_stat==SSLEEP)) {
			a = rp->p_time&0377;
			if(rp->p_nice > NICEFENCE)	/* make her appear older */
				a =* (rp->p_nice - NICEFENCE + 1);
			if(a > n) {
				p1 = rp;
				n = a;
...
found1:
	spl0();
	if((rp->p_flag&SGONE) == 0)
		thrash++;
	rp->p_flag =& ~(SLOAD|SGONE);
	xswap(rp, 1, 0);
	goto loop;
...
data:
	rp = p1;
	size = rp->p_size;
	if(swap(rp->p_addr, a, size, B_READ)) {
		printf("Can't swap data\n");
		badimage++;
		size = USIZE;
		if(swap(rp->p_addr,a,size,B_READ)) {
			printf("Can't swap user\n");
			...
			*ka6 = a;
			u.u_dirp = "/";
			u.u_procp = rp;
			u.u_ruid = u.u_uid = rp->p_uid;
			u.u_cdir = namei(&schar,0);
			*ka6 = saveka6;
			spl0();
		}
	}
	if(!fakeu)
		mfree(swapmap, (size+7)/8, rp->p_addr);
	rp->p_addr = a;
	rp->p_flag =| SLOAD;
	rp->p_time = 0;
	if(badimage) {
...
loop:
	 ...
	 */
	rp->p_flag =| SGONE;	/* tell that she got some time */
	retu(rp->p_addr);
	sureg();
	/*
...
expand(newsize)
{
	...
	for(i=0; i<n; i++)
		copyseg(a1+i, a2++);
	mfree(coremap, n, a1);
	p->p_flag =| SGONE;	/* tell that she got some time */
	retu(p->p_addr);
	sureg();
}
