...
nice()
{
	...
	n = u.u_ar0[R0];
	rn = u.u_procp->p_nice;
	if((n < 0 || (rn > 31+NICEFENCE-1 && rn > n)) && !suser())
		return;
	u.u_procp->p_nice = n;
}
...
/*
 *	system id system call
 *
 */
/* the following fields must match the sysid.h file */
char sidtype[2] SIDTYPE;
int sidnum SIDNUM;
char sidtitle[78] SIDTITLE;
char sidclass[2] SIDCLASS;
int sidosver SIDOSVER;
int sidosed SIDOSED;
int sidchg SIDCHG;
int sidcpu SIDCPU;
int sidfp SIDFP;
int sidndisk SIDNDISK;
int sidntape SIDNTAPE;
int sidntty SIDNTTY;
...
sysid() {
	...
	bp = u.u_arg[0];
	ip = &sidtype[0];
	for(i=0;i<50;i++) {	/* for sizeof sysid.h structure (in words) */
		suword(bp,*ip++);
		bp++;
	}
}
