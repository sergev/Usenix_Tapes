...
int *p;
{
	...
	xccdec(rp->p_textp);
/*
 * If swap out fails, try again, until out of swap space.
 * WITHOUT reallocating the blocks that can't be written to.
 */
	for(rp->p_flag =| SLOCK;;) {
		a = salloc(rp->p_size);
...
int *ip;
{
	...
	rp = u.u_procp;
	rp->p_flag =| SLOCK;
	while(swap(xp->x_daddr, rp->p_addr+USIZE, ts, 0)) {
		printf("Xalloc err\n");
		xp->x_daddr = salloc(ts);
	}
	rp->p_flag =& ~SLOCK;
...
/*
 * allocate swap space in 64 byte chunks
 * panic:  out of swap space
 */
salloc(size)
int size;
{
	register int a;
	if((a = malloc(swapmap,(size+7)/8)) == NULL)
		panic("Out of swap\n");
	return a;
}
