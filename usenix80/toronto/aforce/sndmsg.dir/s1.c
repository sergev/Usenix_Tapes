...
char *replyhdr ".replyhdrxxxxx";
char *replysub ".replysubxxxxx";
char *savlttr;
main (argc, argv)
...
/*	flag.erase = argc-one;	/* one or more arg sets flag.erase, causing a
	...
	puts(1,"Type letter:\n- - -\n"); lttr = input(0,0);  /* input letter */
		...
		lttr = input(subfd,0);
		bomem = lttr;
		puts(1,"Header of original message has been included\nType reply:\n- - -\n");
		input(0,0);
		lttr = savlttr;
	}
	...
	if (flag.unsent)
	{
		...
		if (myfd < 0)
		{
			puts(2,"Can't create unsent_mail\n");
			exit(EACCES);
		}
		/* copy letter */
		seek (fd.tb,0,0);
...
Algorithm:
	...
	CAVEAT:  Not yet using process id number, just xxxxx'x.
