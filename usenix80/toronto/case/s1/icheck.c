 ...
 * record total inodes and total free blocks
 * in superblock with -s option
 */
...
makefree()
{
	register char *i;  /* RAND change for big file systems */
	register int j;    /* RAND ditto */
	...
	sblock.s_fmod = 0;
	sblock.s_tfree = 0;
	sblock.s_tinode = ninode;
	free(0);
	for(i=sblock.s_fsize-1; i>=sblock.s_isize+2; i--) {
		if ((bmap[j = ((i>>4)&07777)] & (1<<(i&017)))==0) { /* RAND */
			free(i);
			sblock.s_tfree++;	// Case
		}
	}
