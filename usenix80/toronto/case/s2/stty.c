...
char	*argv[];
{
	...
	while(--argc > 0) {
		...
		arg = *++argv;
		if (eq("ek")||eq(upper("ek")))
			mode[1] = '#@';
		if (eq("erase")||eq(upper("erase"))) {
			mode[1].lobyte = **++argv;
			argc--;
		}
		if (eq("kill")||eq(upper("kill"))) {
			mode[1].hibyte = **++argv;
			argc--;
		}
		for(i=0; speeds[i].string; i++)
			if(eq(speeds[i].string)||eq(upper(speeds[i].string)))
				mode[0] = speeds[i].speed;
		for(i=0; modes[i].string; i++)
			if(eq(modes[i].string)||eq(upper(modes[i].string))) {
...
prmodes()
{
	...
	printf("erase = '%s'; ",asciize(mode[1].lobyte));
	printf("kill = '%s'\n", asciize(mode[1].hibyte));
	m = mode[2];
...
asciize(value)
{
static char equivalent[3];

	if(value > 040 && value != 0177){
		equivalent[0] = value & 0377;
		equivalent[1] = '\0';
	}else{
		if(value < 033 && value){
			equivalent[0] = '^';
			equivalent[1] = value + 'A' - 1;
			equivalent[2] = '\0';
		}else
			switch(value){
...
{
static char upper_eqiv[MAXMATCH+2];
register char *pup upper_equiv;
