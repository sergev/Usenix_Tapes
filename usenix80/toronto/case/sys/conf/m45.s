...
PROF	= 0		/ include system profiler
...
/ clear bss in D space
	...
	mov	$stk+2,sp
	mov	$05,SSR3		/ 18-bit, K+U sep i and d
	inc	SSR0
	mov	$_etext,r0
	mov	$_edata,r1
	add	$_etext-8192.,r1
...
_idle:
	mov	PS,-(sp)
//////////	sex appeal begins here	//////////
.globl	sexcount,sexdisp
	sub	$1,sexcount		/ decrement shift counter
	bgt	1f			/ wait for it to go to zero
	mov	$4,sexcount		/ reinitialize the counter
	rol	sexdisp			/ shift the display
	bpl	1f			/ if no carry out, we're done
	bis	$1,sexdisp		/ if carry, circulate it back around
1:
//////////	sex appeal ends here	//////////
	spl	0
	wait
_waitloc:
	mov	(sp)+,PS
