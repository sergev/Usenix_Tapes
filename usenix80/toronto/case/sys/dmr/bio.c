...
/* #define	RH	RH	/* define if rh used */
/* #define	UBMAP	UBMAP	/* define if Unibus map used */
...
#ifdef	MONITOR
	if (I.inst_go) {
		I.swp_cnt++;		/* bump total number of swaps */
		I.swp_wds =+ (count<<5);	/* inc tot. # of wds. swapped */
	}
#endif
	fp = &swbuf.b_flags;
	spl6();
...
/*
 * Bclose seems to really blow away our system.  It causes race conditions
 * and other bad things to happen when used with the RK's (root and swap).
 * Purdue intended for DECtape and they only seem to use it for that, not
 * disks.  Anyway, let's remove this code until we check it out more
 * thoroughly.
 * Bill Shannon - 06/14/79
 */
#ifdef	BCLOSE
/*
 ...
 * validating them.
 */
physio(strat, abp, dev, rw, nblocks)
struct buf *abp;
int (*strat)();
unsigned nblocks;
{
	 ...
	 * logical device;
	 */
	off = lshift(u.u_offset, -9);
	if (nblocks) {
		if (off >= nblocks)
			return;
		base = ldiv(u.u_count+511, 512);	/* amt to read/write */
		nblocks =- off;				/* blks left this filesys */
		if (base > nblocks) {
			if (rw == B_WRITE) {
				u.u_error = ENXIO;
				return;
