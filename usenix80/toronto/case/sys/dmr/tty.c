...
/*
 *	Modified for 8 bit raw I/O
 *
 *	Bill Shannon	01/03/78
 */
#include "../param.h"
#include "../systm.h"
#include "../user.h"
...
struct tty *atp;
{
	...
	tp = atp;
	spl5();
	while (tp->t_delct==0 && tp->t_canq.c_cc==0) {
...
struct tty *atp;
{
	...
	if ((t_flags&RAW) == 0) {
		...
		case CUFREEZ:
			...
			return;
		case CFLUSH:
			if (((tp->t_state =^ FLUSH)&FLUSH) == 0)
				ttstart(tp);
			else
				while (getc(&tp->t_outq) >= 0);
			return;
		}
		if (tp->t_rawq.c_cc >= TTYHOG - 8) {
			/* warn him that he's close to the end */
			putc(CBELL, &tp->t_outq);
			ttstart(tp);
		}
		putc(c, &tp->t_rawq);
		if (c == '\n' || c == CEOT || 
			(t_flags&BRKCTRL && c < 040)) {
			wakeup(&tp->t_rawq);
			if (putc(0377, &tp->t_rawq)==0)
				tp->t_delct++;
		}
		if (c == CERASEC && c == tp->t_erase && t_flags&ECHO) {
			ttyoutput(CBACK, tp);
			ttyoutput(' ', tp);
			ttyoutput(CBACK, tp);
			ttstart(tp);
		...
		if (c == CKILLC && c == tp->t_kill && t_flags&ECHO) {
			ttyoutput('^', tp);
			ttyoutput(CKILLC|0100, tp);
			ttyoutput('\n', tp);
			ttstart(tp);
			return;
		}
	} else {
		/* if no mapping was done and all 8 bits wanted, */
		if ((c == (ac&0177)) && ((t_flags&(ODDP|EVENP)) == 0))
			c = ac;		/* then keep all 8 bits */
		putc(c, &tp->t_canq);
		wakeup(&tp->t_rawq);
	}
	if (t_flags&ECHO && !(t_flags&BRKCTRL && c < 040)) {
		ttyoutput(c, tp);
		ttstart(tp);
	}
}
...
struct tty *tp;
{
	...
	if ((rtp->t_flags&RAW) == 0)
		c =& 0177;	/* mask to 7 bits if not raw mode */
	/*
	 * Ignore EOT and ETX (XMIT) in normal mode
	 * to avoid hanging up certain terminals.
	 */
	if ((c==004 || c==027) &&
		 ((rtp->t_flags&RAW)==0 || rtp->t_flags&(ODDP|EVENP)))
		return;
	/*
