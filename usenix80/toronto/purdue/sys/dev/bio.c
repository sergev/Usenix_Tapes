 ...
 * swapping.
 *
 * If running with XBUF defined, there are 2 sets of buffers allocated.
 * one set of number NBUF1 is allocated below and
 * may be addressed conventionally or via ka5.  The other set
 * of number NBUF2 can only be addressed via ka5 and are allocated
 * after the bss and before the text during the "core shuffle"
 * in conf/m45.s.  NBUF2 do not reside in Kernel address space,
 * thereby freeing up Kernel data address space for other uses
 * and allowing a very large NBUF2 pool to be defined if needed.
 * Currently, getblk(NODEV) will always return a buffer from the
 * NBUF1 pool, such that it may be addressed either way.  Buffers
 * on NODEV are used for such things as superblocks and scratch
 * areas for various device drivers.
 * --ghg 01/22/78.
 */
# ifndef XBUF
...
# endif
# ifdef XBUF
char	buffers[NBUF1*512+63]; /* actual buffs start on 64 byte boundry */
# endif
struct	buf	swbuf;
...
# ifdef XBUF
	for(bp=bfreelist.av_forw; bp != &bfreelist; bp = bp->av_forw) {
		if (dev == NODEV2 || dev >= 0 || (bp->b_flags&B_XBUF) == 0)
			goto snarf;
	}
	bfreelist.b_flags =| B_WANTED;
	sleep(&bfreelist, PRIBIO+2);
...
# ifdef XBUF
		if (i < NBUF1) {
			ba = &buffers;
			bp->b_addr = ((ba+63) & ~077) + (i<<9);
			bp->b_par = (bp->b_addr >> 6) & 01777;
		}
		else {
			j = i - NBUF1;
			bp->b_addr = (bufaddr<<6) + (j<<9);
			bp->b_xmem = ((bufaddr + (j<<3)) >> 10) & 077;
			bp->b_par = bufaddr + (j<<3);
		}
		clrbuf(bp);
# endif
		bp->b_back = &bfreelist;
...
/*
 * This is a kludge to allow only 1 device of a set
 * to be do a transfer at a time.  Works much like
 * lock code in mapalloc().  Handler strategy routines
 * of flakey device should call devlock() to avoid
 * multiple transfers.
 * --ghg 10/07/78.
 */
 ...
 *
 * Changed Unibus map for 11/70 to always map first
 * 64KW to physical, instead of 32KW.  This allows
 * the extended region to reach to 64KW.
 * The first 16 mapping registers are preset
 * to physical. The next 8 are allocated and setup
 * by mapalloc(), and the last 7 are currently unused.
 * If allocation is ever changed, be sure  to 
 * update main.c/setum() and power fail code in mch.s
 *--ghg 08/13/78.
 */
int	maplock;
mapalloc(abp)
struct buf *abp;
{
	...
	offset = 0;
	abp->b_xmem = 2;
	for (up = &UBMAP->r[32]; up < &UBMAP->r[48]; up =+ 2) {
		*up++ = i;
		*up-- = a;
		dpadd2(up, offset);
		offset =+ 020000;  /* 4kw */
