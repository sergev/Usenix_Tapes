...
int htdebug 0;			/*  nonzero to debug  */
int htdbg 1;			/* nonzero to debug error recovery */
int htignor;			/* if nz, ignore fatal errors */
int htigner;			/* if nz, ignore all errors */
...
# ifdef POWERFAIL
	if (dev == NODEV) {
		if (httab.d_actf == 0) return;
		HTADDR->htcs1 = IENABLE|REW|GO;	/* abort things */
		HTADDR->htas = -1;
		return;
	}
# endif
...
htstart()
{
	...
	if(h_openf[unit]&FATERR)
	  goto abort;
	if (HTADDR->htds&EOT && (bp->b_flags&B_READ) == 0)
		goto abort;	/* don't write past EOT */
	if (blkno == bp->b_blkno) {
		httab.d_active = SIO;
...
/*  Debugging tool for error recovery ........................ */
	if((HTADDR->htcs1&TRE) && htdbg && (HTADDR->hter != FCE)) {
	  errlg++; /* send message to errlog device, not console */
	  printf("TU16 ERR - cs%o wc%o ba%o fc%o cs%o ds%o er%o cc%o bn%d\n",
		HTADDR->htcs1, HTADDR->htwc, HTADDR->htba,
		HTADDR->htfc,  HTADDR->htcs2,HTADDR->htds,
		HTADDR->hter, HTADDR->htck, bp->b_blkno);
	  errlg = 0; /* set printf back to console */
	}
/* ........................................................... */
	/*  Handle errors and TM's whenever not backing
	    over one (cooked TM recovery).		*/
	if (htigner)
		goto noerr;	/* for working on busted hardware */
	if ((HTADDR->htcs1&TRE) ||
...
/*	      HTADDR->htds&EOT    ||	/* for now... */
		...
		errlg++; /* send error mesg to errlog device */
		deverror(bp, HTADDR->hter, HTADDR->htcs2);
		errlg = 0; /* redirect to console */
		bp->b_flags =| B_ERROR;
	    }
		...
		case SIO:
			...
			if ((bp->b_flags&B_READ)==0 ||
			  ...
			  if (httab.d_errcnt++ < 10)
			   ...
			   {    errlg++; /* send mesg to errlog device */
			    	deverror(bp, HTADDR->hter, HTADDR->htcs2);
				errlg = 0;
				if(!htignor)
					bp->b_flags =| B_ERROR;
...
{
	htphys(dev);
	if (u.u_count == 0) { /* WEOF on zero byte write --ghg 6/10/79 */
		hcommand(dev,WEOF);
		return;
	}
