...
# ifdef XBUF
	*ka5 = bp->b_par;
	ba = &b;
	bcopy(ba+(u.u_offset[1]&0777), &u.u_dent, (DIRSIZ+2)/2);
# endif
...
ret:
	if (!u.u_uid)
		return(dp);
	if (u.u_uid == nuid || (nspy && sflag)) {
		sprintf("nami:%d uid:%d cd:%d/%d %d ino:%d/%d %d ",
		flag, u.u_uid, u.u_cdir->i_dev.d_major,
		u.u_cdir->i_dev.d_minor&0377, u.u_cdir->i_number,
		dp->i_dev.d_major, dp->i_dev.d_minor&0377,
		dp->i_number);
