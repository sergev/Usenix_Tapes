...
/*
 * Scheduling penalty to be added to p_cpu for each block write.
 * This is to keep people in hand who do excessive numbers
 * of block writes (like Tim Wallace).  --ghg 02/04/78
 */

int	sysched	1;	/* scheduling penalty for each block write */
...
struct inode *aip;
{
	...
	do {
		...
		if(u.u_error != 0)
			brelse(bp); else
		if ((u.u_offset[1]&0777)==0 && ip->i_nlink) {
# ifdef XBUF
			*ka5 = praddr;
# endif
			if((i = (u.u_procp->p_cpu&0377) + sysched) > 255)
				i = 255;
			u.u_procp->p_cpu = i;
			bawrite(bp); 
		}
...
# endif
# ifdef XBUF
	if (bp->b_flags & B_XBUF) {
		*ka5 = bp->b_par;
		cp = &b + o;
	}
	else 
		cp = bp->b_addr + o; /* don't map kernel space buf */
# endif
