...
# endif
	...
	rp = u.u_procp;
	if (pri == 0 || chan == 0)
		panic("Bad sleep call");
	if(pri >= 0) {
		if(issig())
			goto psig;
		spl6();
		rp->p_wchan = chan;
/* give better service to procs with favorable nice pri */
/* -ghg 09/27/77 */
		if(rp->p_nice <= -5)
			rp->p_stat = SSLEEP;
		else
...
/*
 * Place the process on the runnable list.
 *
 * Proc is linked to end of list instead of head as in PWB
 * Unix.  This solves problem of several proc's at fixed pri
 * (like -50 for disk I/O) not getting equal service.  It
 * would tend to switch back and forth between two proc's
 * instead of round robin at any given pri level.
 * --ghg 06/16/78.
 *
 * If XBUF, *ka5 must be first setup by caller
 */
setrq(p)
struct proc *p;
{
...
# endif
	...
	pp = up;
	p = (pp->p_cpu & 0377)/8;
	p =+ PUSER + pp->p_nice;	/* no core penalty anymore */
	if(pp->p_flag&LSCHD)
		p =+ 13;
	if(p > 127)
		p = 127;
...
# endif
	...
	if(rp->p_stat==SRUN && (rp->p_flag&SLOAD)==0 &&
            rp->p_time > n && (rp->p_nice != 127 || rp->p_pri < PUSER)) {
#endif
#ifdef XSWAP
	if((rp->p_stat==SRUN && (rp->p_flag&SLOAD)==0 &&
	   rp->p_time > n && (rp->p_nice != 127 || rp->p_pri < PUSER)) || 
	   (rp->p_stat==SWAIT &&
	   (rp->p_flag&SLOAD) == 0 && rp->p_time > ptimx &&
	   rp->p_addr < paddrx)) {
#endif
		p1 = rp;
		n = rp->p_time;
	...
	if(n == -1) {
#ifdef XSWAP
		if(runstg)	/* finish up stage out */
			for(rp = &proc[0]; rp < &proc[NPROC]; rp++)
				if((rp->p_flag&(SLOAD|STGOUT)) ==
				  (SLOAD|STGOUT))
					goto found1;
			runstg = 0;
#endif
	 ...
	 * boot out oldest SWAIT or SSTOP guy which has been in over
	 * 3 seconds.  This help out interactive editors, etc.
	 */
	...
	rp = p1;
	if((rp->p_flag&LSCHD) && n < 120) {
		n = -1;
		for(rp = &proc[0]; rp < &proc[NPROC]; rp++)
			if(rp->p_stat==SRUN && (rp->p_flag&SLOAD)==0 &&
			   rp->p_time > n && (rp->p_flag&LSCHD) == 0) {
				p1 = rp;
				n = rp->p_time;
			}
		if(n == -1)
	...
	for(rp = &proc[0]; rp < &proc[NPROC]; rp++)
		if((rp->p_flag&(SSYS|SLOCK|SLOAD))==SLOAD &&
		   (rp->p_stat==SRUN || rp->p_stat==SSLEEP))
				if((rp->p_flag&LSCHD) == 0 && rp->p_time > n) {
					p2 = rp;
					n = rp->p_time;
				}
				else {
...
* Same as swtch(), but leaves current proc in a runnable
* state.
*/
qswtch()
{
 ...
 */
int	nswtch;	/* for tweeking scheduler */
swtch()
{
	register n;
