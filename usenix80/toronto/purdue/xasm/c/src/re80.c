...
#define NINTVAR 12	/* number of internal varibols	*/
#define NUMEXT 500	/* max # of external referances */
#define NINCL 10	/* max # of nested 'include's	*/
#define NUMSYM 500	/* max. # of symbols	*/
#define NUMGLOB 100	/* max. # of globals	*/
#define NUMLOAD 25	/* max. # of force-loaded moduales	*/
#define	NUMMAC 70	/* max # of macros	*/
#define	NCIM 700	/* max # of charactors in a macro	*/
#define MAXED 60	/* max # of external variables		*/
/*
*
*	8080 relocatible macro assembler
*
*	Peter D. Hallenbeck
*
...
*/
	...
	int exatab[NUMEXT];	/* ext. ref. tab. phys. addr in prog.*/
	int globs[NUMGLOB];	/* global variables	*/
	char gglobs[NUMGLOB*7];
	int loadtab[NUMLOAD];	/* forced moduals load table	*/
...
/*			*/
	...
	numerrs = 0;
	nestc = 0;
	macflag = listflag = 1;	/* turn on listings again	*/
	goto foobar;
}
...
canlist()
{
	return((ifdc== 0)&&(listflag)&&((nestc && (macflag == 0))==0));
}
   ...
   lop7:  i = sscan(arg,ss);
	if(arg[0] == '\0')pcommer(j);
		...
		if(llu(arg,loadtab) != -1){
		    panic("Warning -double declared load file");
		    printf("%%%%\t%s\n",arg);
		    }
...
globlop: sscan(arg,ss);
   ...
   switch(t[j]){
	...
	b2 = i&0377;
	b3 = ((i & 0177400) >> 8 ) & 0377;
	enteref();		/* check for relocatible stuff */
	goto dumper;
