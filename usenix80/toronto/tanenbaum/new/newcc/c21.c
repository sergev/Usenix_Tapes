 ...
 *
 * VU PATCH1: repaired a stupid bug; see big comment below
 * VU PATCH2: another bug in the same procedure dest()
 */
...
#ifdef PATCH1
/*
 * The next procedure was changed by Hans van Staveren, because the
 * optimiser produced wrong code:
 * The sequence:
 *		mov	*6(r5),r4
 *		add	$2,6(r5)
 *		mov	*6(r5),r3
 * had it's last instruction changed by the optimiser into
 *		mov	r4,r3
 * because the source-strings in the first and third instruction were 
 * identical.
 * The change now made is the folllowing:
 * If some destination is changed, indirect this destination is also changed.
 */
#endif
dest(as, flt)
char *as;
