...
/* #define MOD3	1 */
/*
 * VU MOD1: allow defines with an empty replacement
 * VU MOD2: increase the define table; make its size a define
 * VU MOD3: implement -o flag
 * VU PATH: maintain path names on file "local.h". this mod is not marked.
 */
char	sbf[SBSIZE];
/* C command */
...
char *argv[]; {
	...
	int dexit();
	register char	*tmp_path;
	register int	tmp_last;
	...
	if ((signal(2, 1) & 01) == 0)
		signal(2, &dexit);
	(tmp1 = copy(tmp0))[tmp_last-1] = '1';
	(tmp2 = copy(tmp0))[tmp_last-1] = '2';
	(tmp3 = copy(tmp0))[tmp_last-1] = '3';
	if (oflag)
		(tmp5 = copy(tmp0))[tmp_last-1] = '5';
	if (pflag==0)
		(tmp4 = copy(tmp0))[tmp_last-1] = '4';
	for (i=0; i<nc; i++) {
		if (nc>1)
			printf("%s:\n", clist[i]);
		av[0] = "c0";
...
char f[], *v[]; {
	...
	if ((t=fork())==0) {
		/*
		 * try /bin before /usr/bin
		 * and /lib before /usr/lib
		 */
#ifdef PATH
		execv(f+4, v);
#endif
