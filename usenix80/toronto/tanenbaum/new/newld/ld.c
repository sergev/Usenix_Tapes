 ...
 *
 * VU MOD1: large files can now be loaded (long seeks)
 * VU MOD2: -o flag
 * VU MOD3: unique l.out file
 * VU MOD4: library in /usr/lib and long library names allowed e.g. -lncc
 * VU MOD5: increased symbol table
 * VU MODE: mode 755
 * VU PATH: /usr/tmp/....
 */
...
#endif
#ifdef MOD1
	dseek(&reloc, bno, off+uhalf(filhdr.tsize+filhdr.dsize), filhdr.tsize);
#endif
...
#endif
#ifdef MOD1
	dseek(&text, bno, off+uhalf(filhdr.tsize), filhdr.dsize);
	dseek(&reloc, bno, off+filhdr.tsize+uhalf(filhdr.dsize), filhdr.dsize);
#endif
