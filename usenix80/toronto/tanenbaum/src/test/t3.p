...
type wavelength = (red,blue,yellow,q1,q2,q3,q4,q5,q6,q7,q8,q9,q10,q11,
  ...
  vrec = record case t:integer of 0:(r:real); 1:(b:bit) end;
...
begin t:=34; pct := pct + 1;
  rewrite(f1);
  if (not eof(f1)) or eoln(f1) then e(1);
  ...
  if (not eof(f1)) or eoln(f1) then e(4);
  rewrite(f1);
...
begin t:=36; pct:=pct+1;
  ...
  reset(f1);  for i:= 1 to 13 do begin c[i]:= f1^; get(f1) end;
  if (c[2]<>'0') or (c[3]<>'.') or (c[5]<>'6') then e(22);
  if (c[8]<>' ') or (c[10]<>'0') or (c[11]<>'.') or (c[13]<>'6') then e(23);
end;
...
var g1: text;
 ...
 begin t:=35; pct := pct + 1;
  ...
  if (not (eof(g1) and eof(g4))) or eoln(g1) then e(1);
