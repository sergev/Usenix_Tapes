 ...
 * ld.c - link editor
 *
 *	modified 05-Sep-1980 by D A Gwyn:
 *	1) changed create and chmod modes to 0640 & 0750;
 *	2) supports both new and old archive formats;
 *	3) -lx now takes a string for "x";
 *	4) /usr/lib/libx.a tried if /lib/libx.a not found;
 *	5) "-o name" option added for specifying output file name.
 */

#define	SIGINT	2
#define	NARMAGIC 0177545
...
int	newarch;			/* "new archive format" flag */
...
int	infil;
char	*filname;
char	*outname	"a.out";	/* may be changed by -o option */
...
char **argv;
{
	...
	for (c = 1; c<argc; c++) {
		...
		case 'o':		/* output file name */
			if ( ++c >= argc )
				error( 1 , "Bad 'out'" );
			outname = *p++;	/* pointer to name string */
			continue;
...
char *acp;
{
	...
	sizarch = newarch ? sizeof narchdr : sizeof archdr;
	nbno = 0;
	noff = 1;
	for (;;) {
		dseek( &text , nbno , noff , sizarch );
		if (text.size <= 0) {
			libp->bno = -1;
			libp++;
			return;
		}
		mget( newarch ? &narchdr : &archdr , sizarch );
		if ( load1( 1 , nbno , noff + sizarch / 2) )
			{
			libp->bno = nbno;
			libp->off = noff;
			libp++;
			}
		noff =+ ((newarch ? narchdr.a_size[1] : archdr.asize)
			+ sizarch) >> 1;
		nbno =+ (noff >> 8) & 0377;
		noff =& 0377;
	}
...
char *acp;
{
	...
	for (lp = libp; lp->bno != -1; lp++) {
		dseek( &text , lp->bno , lp->off , sizarch );
		mget( newarch ? &narchdr : &archdr , sizarch );
		mkfsym( newarch ? narchdr.a_name : archdr.aname );
		load2( lp->bno , lp->off + sizarch / 2 );
	}
	libp = ++lp;
}
...
char *acp;
{
	...
	cp = acp;
	narchdr.a_name[0] = archdr.aname[0] = '\0';
	if (cp[0]=='-' && cp[1]=='l') {
		cpystr( "/lib/lib" , tmpstr );
		if ( cp[2] )
			cpystr( &cp[2] , &tmpstr[8] );
		else
			cpystr( "a" , &tmpstr[8] );
		append( ".a" , tmpstr );
		if ( (infil = open( tmpstr , 0 )) < 0 )
			prepend( "/usr" , tmpstr );
		else
			close( infil );
		cp = tmpstr;
	...
	if (text.size <= 0)
		error(1, premeof);
	newarch = (c = get( &text )) == NARMAGIC;
	return ( newarch || c == ARCMAGIC );
}

cpystr( from, to )
	char	*from, *to;
	{
	register char	*top, *fromp;
...
char *s;
{
	if (filname) {
		printf("%s", filname);
		if ( narchdr.a_name[0] || archdr.aname[0] )
			if ( newarch )
				printf( "(%.14s)" , narchdr.a_name );
			else
