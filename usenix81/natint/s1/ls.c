...
int     aflg, dflg, lflg, sflg, tflg, uflg, iflg, fflg, gflg, Rflg, args;
int     fout;
int     rflg    1;
char    *year;
int     flags;
...
rpentry(ep, blastp, level)
struct lbuf *ep, *blastp;
{
	register struct lbuf *ep1, *slastp;
	char *dend;
	register char *s;

	if (ep->lflags&DIR && dflg==0 || fflg) {
		if(level>0 && (eqstr(&ep->lname[0],".") || eqstr(&ep->lname[0],"..") )) goto skip;
		for(s = dirbuf; *s; s++ ) ;
		if(s>dirbuf) *s++ = '/';
		dend = s;
		for(s = (level? &ep->lname[0] : ep->namep); *dend = *s++; dend++ ) ;
		if (level==0 && args)
			printf("\n%s:\n", dirbuf);
		if(level>0) pentry(ep, level-1);
		lastp = blastp;
		readdir(dirbuf);
		slastp = lastp;
		if (fflg==0)
			qsort(blastp,slastp - blastp,sizeof *lastp,compar);
		if (Rflg==0 && statreq)
			printf("total %d\n", tblocks);
		for (ep1=blastp; ep1<slastp; ep1++) {
			if (Rflg)
				rpentry(ep1, slastp, level+1);
			else
				pentry(ep1, level);
			*dend = 0;
		}
	} else
skip:           pentry(ep, level-1);
}
