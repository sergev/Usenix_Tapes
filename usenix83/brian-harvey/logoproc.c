...
extern int pauselev, errpause, catching, flagquit;	/* PAUSE */
extern int turtdes;
...
struct object *nameob;
{
	...
	if (pptr=proclook(name)) {
		...
		if (!(pbuf=fopen(s,"r"))) {
			cpystr(s,"/usr/lib/logo/",name,EXTEN,NULL);
			if (!(pbuf = fopen(s,"r"))) {
				onintr(errrec,1);
...
help()
{
	...
	sbuf=fopen("/usr/doc/logo/helpfile","r");
	if (sbuf == NULL) {
...
struct object *arg;
{
	...
	for (lexp = keywords; lexp->word; lexp++)
		if (!strcmp(argstr,lexp->word) || !strcmp(argstr,lexp->abbr))
			break;
	if (!lexp->word) {
	...
	if (strlen(lexp->word) > 9)	/* kludge for Eunice */
		cpystr(fname,"/usr/doc/logo/",lexp->abbr,NULL);
	else
		cpystr(fname,"/usr/doc/logo/",lexp->word,NULL);
