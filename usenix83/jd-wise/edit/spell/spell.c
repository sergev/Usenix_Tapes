 ...
 * accepts text lines from standard input
 * writes misspelled words on standard output
 * if "-E ppid" is given as an argument, send SIGEMT to ppid on error
 * if a line begins with '\001' the remainder of the line is taken
 * to be a word which is to be accepted as correct.
 */
#include <signal.h>
#include "spell.h"
#define DLEV 2
...
char *deriv[40];
char affix[40];
char *dictfile="/usr/lib/edit/hdict";
char *stopfile="/usr/lib/edit/hstop";

main(argc,argv)
	char **argv;
	{
	if(!prime(dictfile,stopfile)) {
		write(2,"spell: cannot initialize hash table\n",36);
		exit(1);
	}
...
badword()
	{
	/*
	printf("%s: %s%s\n",curtab?"stopped":"not found",affix,original);
*/
	printf("%s%s\n",affix,original);
	fflush(stdout);
	if(ppid)
...
find(tabno)
	...
	curtab=tabno;
	affix[0] = 0;
	for(ep=word; *ep; ep++)	/* find end of word */
		;
