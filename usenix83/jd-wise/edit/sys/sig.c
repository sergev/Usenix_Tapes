 ...
 *		note: this implementation is quick but dirty
 */
...
psig()
{
	...
	if (rp->p_flag&STRC)
		stop();
	n = fasig(rp);	/* was fsig	jdw 5-3-81 */
	if (n==0)
		return;
	rp->p_sig &= ~(1<<(n-1));
	if((p=u.u_signal[n]) != 0) {
...
/*
 * find the first "active" signal	jdw 5-3-81
 */
fasig(p)
	struct proc *p;
	{
