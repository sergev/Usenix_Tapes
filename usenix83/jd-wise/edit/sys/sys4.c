 ...
 * modifications:
 *	implemented signal holding	jdw 5-3-81
 */

#define SIG_HOLD 3	/* jdw 5-3-81 */
...
/*
 * Establish vector of groups to which user belongs
 * dhj 2/28/80
 */
...
register struct a {
 ...
 uap = (struct a *)u.u_ap;
 copyin(uap->addr, (caddr_t)u.u_gvec, sizeof u.u_gvec);
 u.u_rgid = u.u_gid = u.u_gvec[0];
}
...
chown()
{
	...
	if ((ip = owner()) == NULL)
		return;
	if (ip->i_uid != uap->uid && !suser())
		goto done;
	ip->i_uid = uap->uid;
	for (i = 0; i < MAXGROUPS && u.u_gvec[i] != -1; i++) {
		if (u.u_gvec[i] == uap->gid || u.u_uid == 0) {
			ip->i_gid = uap->gid;
			break;
			}
		}
...
ssig()
{
	...
	if(u.u_r.r_val1 != SIG_HOLD)	/* jdw 5-3-81 */
