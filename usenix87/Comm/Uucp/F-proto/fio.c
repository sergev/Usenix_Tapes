 ...
 * - the global variable IsTcpIp, when set, excludes ioctl's,
...
#define FOBUFSIZ	1024	/* for X.25 interfaces: set equal to packet size;
...
acct:
	...
	if (retries > 0)
		sprintf(&ibuf[strlen(ibuf)], ", retry #%d", retries);
	DEBUG(1, "%s\n", ibuf);
	syslog(ibuf);
	if (ack == 'R') {
		DEBUG(4, "RETRY:\n", 0);
