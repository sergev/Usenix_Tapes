...
checknak:		switch (readbyte(6)) {
			...
			case TIMEOUT: {
				logit("Timeout while sending filename\n");
				sendbyte(BAD_NAME);
				return (-1);
			}

			default: {
				logit("Error while sending filename\n");
				sendbyte(BAD_NAME);
				return (-1);
			}
		}	
	...
	if (readbyte(10) != cksum) {
		logit("Bad checksum while sending filename\n");
		sendbyte(BAD_NAME);
		return (-1);
	}
