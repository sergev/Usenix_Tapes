     ...
     int inhibit_hairy_id;
{
	...
	if (lnew && lnew != l)
	  {
   	    /* On VMS systems, stdout is not supposed to be buffered. This
   	       means that the following code, which figures what to do based
   	       on the number of characters that have been buffered is
   	       meaningless. */
#ifndef VMS
	    ...
	    int outq = stdout->_ptr - stdout->_base;
