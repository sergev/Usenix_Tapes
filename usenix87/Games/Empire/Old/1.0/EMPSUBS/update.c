...
#ifdef DEBUG
if(verbose) printf("update.c: workforce = %lf\n", workforce);
#endif DEBUG
...
X240:	
	...
	if( (selup & UP_TIME) == 0 ) goto X310;
	iwork = dt * workforce;
#ifdef DEBUG
if(verbose) printf("update.c: !UP_TIME: dt * workforce = %d\n", iwork);
#endif DEBUG
...
X1116:	
#ifdef DEBUG
if(verbose) printf("update.c: doing population growth\n");
#endif DEBUG
	q = sect.sct_civil * dt;
	if( sctdes != S_URBAN ) goto X1256;
#ifdef DEBUG
if(verbose) printf("update.c: URBAN: dt * civil = %d\n", q);
if(verbose) printf("update.c: URBAN: sct_ore = %d\n", (int)sect.sct_ore);
if(verbose) printf("update.c: URBAN: q / 100 = %d\n", q / 100);
if(verbose) printf("update.c: URBAN: ore > q / 100 = %d\n", sect.sct_ore > q / 100);
#endif DEBUG
	q = (sect.sct_ore > q / 100) ? q / 100 : sect.sct_ore;
#ifdef DEBUG
if(verbose) printf("update.c: URBAN: after ore limit = %d\n", q);
#endif DEBUG
	q = max127(sect.sct_civil + q) - sect.sct_civil;
#ifdef DEBUG
if(verbose) printf("update.c: URBAN: after max127 = %d\n", q);
#endif DEBUG
	sect.sct_ore -= q;
	sect.sct_civil += q;
	goto X1372;
...
#ifdef DEBUG
if(verbose) printf("update.c: BSPAN: after reduction, civil = %d\n", (int)sect.sct_civil);
#endif DEBUG
	goto X1372;
X1310:	
	sect.sct_civil = max127(sect.sct_civil + ((sect.sct_civil > 31 && sect.sct_civil < 97) ? q / 200 : q / 400));
#ifdef DEBUG
if(verbose) printf("update.c: otherwise, civil = %d\n", (int)sect.sct_civil);
#endif DEBUG
X1372:	
...
X6030:	
		...
		break;
	case S_MINE:
		q = (float)sect.sct_miner * sect.sct_effic * work / 1.e4;
