...
int findname(name,stat) 	    /* return location of character 'name' */
...
int	loc = 0;
    ...
    while (fread((char *) stat,sizeof(sbuf),1,fp))
...
void	death(stat, how)		/* remove a player after dying */
reg struct  stats   *stat;
char *how;
{
FILE	*fp;
...
long	ltemp;
    ...
    scoreboard(stat);
    fp = fopen(lastdead,"w");
    fprintf(fp,"%s (log: %s lvl: %d typ: %d, by %c%s)",
	stat->name,stat->login,stat->lvl,stat->typ,tolower(*how),how + 1);
    fclose(fp);
    fp = fopen(messfile,"w");
    fprintf(fp,"%s was killed by %c%s.", stat->name,tolower(*how),how + 1);
    fclose(fp);
    initchar(stat);
    strcpy(stat->name,"<null>");
    update(stat,fileloc);
...
int ch;
    ...
    mvaddstr(8,0,"Name                         X         Y       Lvl  Type  Login\n");
    fp = fopen(peoplefile,"r");
    while (fread((char *) &buf,sizeof(buf),1,fp))
	{
	if (buf.status)
...
void	showusers(screen)	    /* print a list of all characters */
bool	screen;
{
struct	stats	buf;
FILE	*fp;
...
int level(expr) 	/* calculate level */
double	expr;
{
    if (expr < 1.1e+7)
...
reg double  *bp;
    ...
    *bp = stat->sin;
    ...
    second = roll(0,5);
    temp1 = bp[first];
    /* this expression is split to prevent a complire loop on some compilers */
    temp2 = bp[second];
    bp[first] = temp2;
    bp[second] = temp1;
