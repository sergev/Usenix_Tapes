	...
	second;
{
    ...
    if ((td = creat (tmpfil, 0600)) == NOTOK) {
	advise (tmpfil, "unable to create");
	goto out1;
    }
    (void) close (td);
    if ((td = creat (cp = map_name (tmpfil), 0600)) == NOTOK) {
...
pick +bbl -before -12 -datefield BB-Posted -sequence select -zero
pick +bbl -before -20 -sequence select -nozero
scan select; refile -src +bbl select +bbl/arc
rmf +bbl/arc
...
/* bbexp.c - generate a script to archive the BBoards area */
...
char  **argv;
{
    ...
    (void) setbbent (SB_STAY);
    while (bb = getbbent ())
	switch (bb -> bb_flags & BB_ARCH) {
	    case BB_ASAV: 
		printf ("bbl -shell bbsave %s\n", bb -> bb_name);
		break;

	    case BB_AREM: 
		printf ("bbl -shell bbremove %s\n", bb -> bb_name);
		break;
	}
