...
check_uucp()
{
	...
	if (chdir(buf) < 0) {
	   fprintf (stderr, "nbatcher: can't chdir to %s - bypassing UUCP check\n", buf);
	   return (TRUE);
	}

	if ((dfp=fopen(".", "r")) == NULL) {
	   fprintf (stderr, "nbatcher: fopen error on %s - bypassing UUCP check\n", UUCPDIR);
	   return (TRUE);
	}
	...
	while ((fread((char *)&dp, sizeof(dp), 1, dfp)) == 1) {
		...
		if (stat(dp.d_name, &st) < 0) {
		  fprintf (stderr, "nbatcher: bad stat on UUCP_file %s - bypassing\n", dp.d_name);
		  continue;
		}
		n_bytes += st.st_size;
