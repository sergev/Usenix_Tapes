...
inputnews() {
      ...
      if (in.nargs == 0)
            goto out ;
      qsort(in.arg, in.nargs, sizeof(char *), comp) ;
      sleep(5) ;		/* in case any files half written */
      for (i = 0 ; i < in.nargs ; i++) {
            procfile(in.arg[i]) ;
      }
      if ((fp = fopen("../ackfile", "a")) == NULL)
            fatal("Can't create ack file") ;
      for (i = 0 ; i < in.nargs ; i++) {
            register char *fname = in.arg[i] ;
            if (! badname(fname))
                  fprintf(fp, "%s\n", fname) ;
            free(fname) ;
      }
...
procnm(name, fp, ismail)
      ...
      if (rc != 0) {
            ...
            fprintf(mailfp, "Subject: %s failed\n\n", ismail? "rmail" : "rnews") ;
            if ((rc & 0177) == 0) {
...
procnews(name, fp)
      ...
      if (run(arg, SETIN, fileno(fp)) != 0) {
            msg("rnews failed, file %s", name) ;
            return -1 ;
      }
      return 0 ;
}
...
setlock(name)
      ...
      if ((fd = creat(name, 0000)) < 0) {
            if (stat(name, &statb) >= 0) {
                  if (time((long *)0) - statb.st_mtime < 4L * 3600L)
                        return 0 ;
                  unlink(name) ;
                  msg("Old lock removed") ;
            }
            if ((fd = creat(name, 0000)) < 0)
                  return 0 ;
      }
