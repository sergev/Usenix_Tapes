...
/* For C++, two types of symbolic output can be performed.  If write_symbols
   is 2, then we output standard dbx-style info, which means the user must
   deal with renamed variables and function names himself.

   If write_symbols_plus is non-zero, the we output dbx+-style info,
   which makes dbx+ handle things automagically.  */

extern int write_symbols, write_symbols_plus;
     ...
     int full;
{
  ...
  switch (TREE_CODE (type))
    ...
    case CLASS_TYPE:
      type = TYPE_MAIN_VARIANT (type);
      warning ("putting out class `%s' as struct", TYPE_NAME_STRING (type));
    case RECORD_TYPE:
    case UNION_TYPE:
	  ...
	  || TYPE_SIZE (type) == 0)
	{
	  /* If the type is just a cross reference, output one
	     ...
	     its real definition.  */
	  fprintf (asmfile, (TREE_CODE (type) == RECORD_TYPE)
		   ? "xs" : "xu");
	  CHARS (3);
      ...
      if (TYPE_BASELINK (type) && write_symbols_plus)
	{
	  putc ('!', asmfile);
	  putc (TREE_VIA_PUBLIC (type) ? '2'
		: TREE_VIA_PROTECTED (type) ? '1'
		: '0',
		asmfile);
	  dbxout_type (TYPE_BASELINK (type), 0);
	  putc (',', asmfile);
	  CHARS (2);
	}
      CHARS (11);
	...
	if (DECL_NAME (tem) != 0)
	  {
	    ...
	    if (write_symbols_plus)
	      {
		putc ('/', asmfile);
		putc (TREE_PRIVATE (tem) ? '0'
		      : TREE_PROTECTED (tem) ? '1'
		      : '2', asmfile);
	      }
	    ...
	    if (TREE_STATIC (tem))
	      if (write_symbols_plus)
		{
		  fprintf (asmfile, ":%s;", IDENTIFIER_POINTER (DECL_STATIC_NAME (tem)));
		}
	      else
		{
		  fprintf (asmfile, ",0,0;");
      ...
      fields = write_symbols_plus ? TYPE_FN_FIELDS (type) : NULL_TREE;
      if (TREE_HAS_CONSTRUCTOR (type) && write_symbols_plus)
	{
	  tree dtor;

	  /* We have farmed out class which have only destructors
	     defined. To wit: if a class has a destructor, then it
	     has at least one constructor.  */
	  if (TREE_HAS_DESTRUCTOR (type))
	    {
	      dtor = TREE_VALUE (fields);
	      tem = TREE_CHAIN (dtor);
	    }
	  else
	    {
	      dtor = NULL_TREE;
	      tem = TREE_VALUE (fields);
	    }
	  CHARS (2);
	  ...
	  if (tem)
		...
		fprintf (asmfile, "op$::%s.", OPERATOR_NAME_STRING (TREE_TYPE (tem)));
	      else
		fprintf (asmfile, "%s::", IDENTIFIER_POINTER (DECL_NAME (tem)));
	      ...
	      while (tem)
		{
		  ...
		  CONTIN;
		  t = TREE_TYPE (tem); /* Get to the FUNCTION_DECL */
		  dbxout_type (TREE_TYPE (t), 0); /* FUNCTION_TYPE */
		  dbxout_args (TYPE_ARG_TYPES (TREE_TYPE (t)));
		  fprintf (asmfile, ":%s;%c%c", IDENTIFIER_POINTER (DECL_NAME (t)),
			   TREE_PRIVATE (tem) ? '0' : TREE_PROTECTED (tem) ? '1' : '2',
			   TREE_VIRTUAL (tem) ? '*' : '.');
		  CHARS (1 + IDENTIFIER_LENGTH (DECL_NAME (t)));
		  if (TREE_VIRTUAL (tem))
		    {
		      fprintf (asmfile, "%d;",
			       TREE_INT_CST_LOW ((tree)DECL_OFFSET (tem)));
		      CHARS (4);
		    }
		  if (tem == dtor)
		    break;
      ...
      for (; fields; fields = TREE_CHAIN (fields))
	{
	  ...
	  if (tem)
		...
		fprintf (asmfile, "%s::", IDENTIFIER_POINTER (TREE_PURPOSE (fields)));
		   ...
		   with its parameter list */
		{
		  ...
		  fprintf (asmfile, ":%s;%c%c", IDENTIFIER_POINTER (DECL_NAME (t)),
			   TREE_PRIVATE (tem) ? '0' : TREE_PROTECTED (tem) ? '1' : '2',
			   TREE_VIRTUAL (tem) ? '*' : '.');
		  CHARS (4 + IDENTIFIER_LENGTH (DECL_NAME (t)));
		  if (TREE_VIRTUAL (tem))
		    {
		      fprintf (asmfile, "%d;",
			       TREE_INT_CST_LOW ((tree)DECL_OFFSET (tem)));
      ...
      if (write_symbols_plus)
	{
	  ...
	  if (TREE_VIRTUAL (type))
	    {
	      extern tree get_vfield_name ();
	      tree t, v = get_vfield_name (type);
	      t = type;
	      while (TYPE_BASELINK (t) && TREE_VIRTUAL (TYPE_BASELINK (t)))
		t = TYPE_BASELINK (t);
	      putc ('%', asmfile);
	      CHARS (1);
	      dbxout_type (t, 0);
	      fprintf (asmfile, ",%s;", IDENTIFIER_POINTER (v));
	      CHARS (sizeof (",%s;") + IDENTIFIER_LENGTH (v));
	    }
	  else
	    {
	      putc (';', asmfile);
    ...
    case REFERENCE_TYPE:
      putc (write_symbols_plus ? '&' : '*', asmfile);
      CHARS (1);
      dbxout_type (TREE_TYPE (type), 0);
      break;
...
/* Output a definition of a typedef name.
   ...
   Output nothing it TYPE's definition has been output already.  */
     ...
     tree type;
{
  ...
  fprintf (asmfile, ".stabs \"%s:t", TYPE_NAME_STRING (type));
  dbxout_type (type, 1);
  dbxout_finish_symbol ();
}
