...
extern void cleanup_temporary_aggregates ();
...
#define DEFTREECODE_X(SYM, NAME, TYPE, LENGTH, ERR_NAME) 0,
#define DEFTREECODE_XY(SYM, NAME, TYPE, LENGTH, ERR_NAME, ASG_ERR_NAME) 0,
     ...
     va_dcl
{
  ...
  if (args_size != 0)
    argblock = push_block (round_push (gen_rtx (CONST_INT, VOIDmode, args_size)));
#endif
     ...
     enum expand_modifier modifier;
{
  ...
  switch (code)
    ...
    case RESULT_DECL:
      if (DECL_RTL (exp) == 0)
	{
	  if (temp_name_p (DECL_NAME (exp)))
	    {
	      /* Certain temporaries cannot be initialized until
		 we know that they can be used.  */
	      expand_decl (exp, ! TREE_HAS_CONSTRUCTOR (exp));
	      if (DECL_RTL (exp) == 0)
		abort ();
	    }
	  else
	...
	if (TREE_CODE (exp) == SCOPE_REF)
	  {
	    /* There are currently three reasonable things that
	       `resolve_scope_ref' can return: error_mark_node,
	       a VAR_DECL, or a COMPONENT_REF.  In case we
	       ever want to return anything else, just
	       call `expand_expr' again on the new item.  */

	    exp = resolve_scope_ref (exp);
    ...
    case CONVERT_EXPR:
      ...
      /* C++: taking the address of a reference variable
	 merely returns its value.  */
    case REFERENCE_EXPR:
...
/* ??? cv's were used to effect here to combine additive constants
   ...
   Also, the addition of one can be handled by changing the condition.  */
	...
	if (TREE_GETS_INIT_REF (type))
	  {
	    abort ();
	    exp = build_classfn_ref (lhs, DECL_NAME (TYPE_NAME (type)),
				     build_tree_list (0, convert_to_reference (0, TYPE_REFERENCE_TO (type), rhs, 0)), 1, 1);
	    if (exp == error_mark_node)
	      return const0_rtx;
	    /* Initialization via a constructor makes the return
	       value effectively void type.  */
	    temp = expand_expr (exp, 0, VOIDmode, EXPAND_NORMAL);
	    return temp;
	  }
	...
	/* If lhs is complex, expand calls in rhs before computing it.
	   ...
	   C++: we cannot perform a bitwise copy operation on
	   aggregates which contain virtual function table pointers.
	   Rather, we save to a temporary the virtual function
	   table pointer, perform the bitwise copy, and then
	   restore the pointer.  */
	tree lhs = TREE_OPERAND (exp, 0);
	tree rhs = TREE_OPERAND (exp, 1);
	tree type = TREE_TYPE (lhs);
     ...
     int ignore;
{
  ...
  if (TYPE_MODE (TREE_TYPE (exp)) == BLKmode)
	...
	/* Make room on the stack to hold the value.
	   ...
	   C++: if this type has a destructor, then we must
	   allocate space which can be destroyed.  */
	if (TREE_NEEDS_DESTRUCTOR (TYPE_MAIN_VARIANT (TREE_TYPE (exp))))
	  {
	    tree decl = get_temp_name (TREE_TYPE (exp), error_mark_node, exp);
	    expand_decl (decl, 0);
	    if (DECL_RTL (decl) == 0)
	      abort ();
...

/* These probably belong in emit-rtl.c if they belong anywhere.  However,
   by placing them here, we can use GNU CC's emit-rtl.c file without
   any modifications.  */
rtx
next_insn (insn)
     rtx insn;
...
void
emit_use_insn (result)
     tree result;
{
  if (REG_P (DECL_RTL (result)))
    emit_insn (gen_rtx (USE, VOIDmode, DECL_RTL (result)));
}
