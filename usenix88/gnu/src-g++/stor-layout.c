     ...
     tree rec;
{
...
#endif
  tree basetype, baselink;
  /* For C++, these must be laid out *after* the record is.  */
  ...
  for (field = TYPE_FIELDS (rec); field; field = TREE_CHAIN (field))
	...
	{
	  /* lay this into the real world */
	  tree decl;

	  decl = IDENTIFIER_GLOBAL_VALUE (DECL_STATIC_NAME (field));
#if 0
	  /* we should not need to do this, unless we *really*
	     want pointers to static members.  */
	  layout_decl (field, 0);
#endif
	  pending_statics = tree_cons (DECL_ASM_NAME (field), decl, pending_statics);
	  continue;
	}
	      ...
	      && var_size))
	{
      ...
      if (TREE_ANON_UNION_ELEM (field))
	{
	  tree last_field = field;
	  int vconst_size = 0;
	  tree vvar_size = NULL_TREE;
	  int union_align = BITS_PER_UNIT;

	  /* Do a little layout-union in here.  */
	  for (; field && TREE_ANON_UNION_ELEM (field);
	       last_field = field, field = TREE_CHAIN (field))
	    {
	      if (TREE_STATIC (field))
		continue;
	      DECL_OFFSET (field) = const_size;
		...
		vconst_size = MAX (vconst_size,
				   TREE_INT_CST_LOW (DECL_SIZE (field))
				   * DECL_SIZE_UNIT (field));
	      else if (vvar_size == 0)
		vvar_size = convert_units (DECL_SIZE (field),
					   DECL_SIZE_UNIT (field),
					   BITS_PER_UNIT);
	      else
		vvar_size = genop (MAX_EXPR,
	    ...
	    }
	  {
	    ...
	    /* Now add this size to the size of the structure.  */
	    if (NULL == vvar_size)
	      dsize = build_int (CEIL (vconst_size, BITS_PER_UNIT));
	    else if (vconst_size == 0)
	      dsize = vvar_size;
	    else
	      dsize = genop (MAX_EXPR, vvar_size,
			     build_int (CEIL (vconst_size, BITS_PER_UNIT)));
	    if (TREE_LITERAL (dsize))
	      const_size += TREE_INT_CST_LOW (dsize) * BITS_PER_UNIT;
	    else if (var_size == 0)
	      {
		var_size = dsize;
		size_unit = BITS_PER_UNIT;
	      }
	    else
	      {
		var_size
		  = genop (PLUS_EXPR,
			   convert_units (var_size, size_unit, BITS_PER_UNIT),
			   dsize);
	      }
	    field = last_field;
	    continue;
     ...
     tree rec;
{
  ...
  for (field = TYPE_FIELDS (rec); field; field = TREE_CHAIN (field))
    {
      if (TREE_STATIC (field))
	{
	  error ("field `%s' cannot be declared static in union",
  ...
  /* C++: Do not let empty unions exist.  */
  if (const_size == 0 && var_size == 0)
