...
/* Like symout_strings but only update next_address; do not output anything.  */
...
/* Call here to output a chain of types.
   ...
   C++: Since the type record structure is larger, and contains more
   elements which are likely to be zero, we zero out all of the
   elements in RECORDS ahead of time.  This saves processing later.  */
     ...
     tree types;
{
  ...
  /* C++: zero everything out ahead of time.  */
  bzero (records, n_types * sizeof (struct typerec));
  ...
  for (next = types, i = 0;
	...
	case UNION_TYPE:
	  {
	    /* C++: take care of method info.  */
	    int len = 0;
	    tree tmp = TYPE_FN_FIELDS (next);

	    records[i].has_constructor = TREE_HAS_CONSTRUCTOR (next);
	    records[i].has_destructor = TREE_HAS_DESTRUCTOR (next);
	    records[i].via_protected = TREE_VIA_PROTECTED (next);
	    records[i].via_public = TREE_VIA_PUBLIC (next);

	    while (tmp)
	      {
		len += list_length (TREE_VALUE (tmp));
		tmp = TREE_CHAIN (tmp);
	      }
	    records[i].nfn_fields = len;
	    tmp = next;
	    if (TYPE_BASELINK (tmp))
	      {
		records[i].baseclass = TYPE_SYMTAB_ADDRESS (TYPE_BASELINK (tmp));
		while (TYPE_BASELINK (tmp)
		       && TREE_VIRTUAL (TYPE_BASELINK (tmp)))
		  tmp = TYPE_BASELINK (tmp);
		records[i].vptr_basetype = TYPE_SYMTAB_ADDRESS (tmp);
	      }
	    if (TREE_VIRTUAL (tmp))
	      {
		tree field = TYPE_FIELDS (tmp);
		int len = 0;
		while (! VFIELD_NAME_P (DECL_NAME (field)))
		  {
		    len++;
		    field = TREE_CHAIN (field);
		  }
		records[i].vptr_fieldno = len;
	      }
	    /* C++: fall through.  */
	  }
	case ENUMERAL_TYPE:
      ...
      if (records[i].nfields)
	{
	  ...
	  next_address += records[i].nfields
	    * (sizeof (struct field) + 2 * BITS_PER_WORD);
	}
      if (records[i].nfn_fields)
	{
	  records[i].fn_fieldlists_address = next_address;
	  next_address += records[i].nfn_fields * sizeof (struct fn_fieldlist);
	}
    }
  ...
  for (next = types, i = 0;
	...
	= ((TREE_CODE (next) == INTEGER_TYPE || TREE_CODE (next) == ENUMERAL_TYPE)
      ...
      buffer.has_constructor = records[i].has_constructor;
      buffer.has_destructor = records[i].has_destructor;
      buffer.via_protected = records[i].via_protected;
      buffer.via_public = records[i].via_public;
      buffer.vptr_basetype = (struct type *)records[i].vptr_basetype;
      buffer.vptr_fieldno = records[i].vptr_fieldno;
      buffer.baseclass = (struct type *)records[i].baseclass;
      buffer.main_variant = (struct type *)TYPE_SYMTAB_ADDRESS (TYPE_MAIN_VARIANT (next));
      buffer.nfn_fields = records[i].nfn_fields;
      buffer.nfn_fields_total = 0;
      buffer.fn_fieldlists = (struct fn_fieldlist *) records[i].fn_fieldlists_address;
	...
	case POINTER_TYPE:
	  if (TREE_MEMBER_POINTER (next))
	    buffer.code = TYPE_CODE_MPTR;
	  else
     ...
     tree type;
{
  struct field buffer;
  register tree field;
  int len = list_length (TYPE_FIELDS (type)) / HOST_BITS_PER_INT;
  int *private_bits = (int *)alloca (len);
  int *protected_bits = (int *)alloca (len);

  bzero (private_bits, len);
  bzero (protected_bits, len);

  for (field = TYPE_FIELDS (type), len = 0;
       field;
       field = TREE_CHAIN (field), len++)
    {
	...
	B_SET (private_bits, len);
      else if (TREE_PROTECTED (field))
	B_SET (protected_bits, len);
    }
  fwrite (private_bits, len, 1, symfile);
  fwrite (protected_bits, len, 1, symfile);
}

static void
symout_record_fn_fields (type)
     tree type;
{
  struct fn_fieldlist buffer;
  int len = list_length (TYPE_FN_FIELDS (type));
  int *addrvec = (int *)alloca (len * sizeof (int));
  register tree field, fields;

  for (fields = TYPE_FN_FIELDS (type), len = 0;
       fields;
       fields = TREE_CHAIN (fields), len++)
    {
      int i;
      int len2 = list_length (TREE_VALUE (fields));
      int *addrvec2 = (int *)alloca (len2 * sizeof (int));
      int *private_bits = (int *)alloca (len2 * sizeof (int) / BITS_PER_WORD);
      int *protected_bits = (int *)alloca (len2 * sizeof (int) / BITS_PER_WORD);
      struct fn_field *fn_fields = (struct fn_field *)alloca (len2 * sizeof (struct fn_field));
      struct fn_field *p2 = fn_fields;
      int *my_addrs = (int *)alloca ((len2 + 1) * sizeof (int));
      int **my_args = (int **)alloca (len2 * sizeof (int *));

      bzero (private_bits, len2 * sizeof (int));
      bzero (protected_bits, len2 * sizeof (int));
      my_addrs[0] = 0;
      buffer.name = (char *)next_address;
      buffer.length = len2;
      symout_strings_skip (0, IDENTIFIER_LENGTH (TREE_PURPOSE (fields)), 0, 0);
      buffer.private_fn_field_bits = (int *)next_address;
      buffer.protected_fn_field_bits = (int *)next_address + len2;
      buffer.fn_fields = (struct fn_field *)((int *)next_address + 2 * len2);

      for (field = TREE_VALUE (fields), len2 = 0;
	   field;
	   field = TREE_CHAIN (field), len2++)
	{
	  struct fn_field buffer2;
	  tree argtype;
	  int len3 = list_length (TYPE_ARG_TYPES (TREE_TYPE (field)));
	  int *addrvec3 = (int *)alloca (len3 * sizeof (int));
	  int *p3 = addrvec3;

	  for (argtype = TYPE_ARG_TYPES (TREE_TYPE (field));
	       argtype;
	       argtype = TREE_CHAIN (argtype))
	    *p3++ = TYPE_SYMTAB_ADDRESS (TREE_VALUE (argtype));

	  p2->type = (struct type *)TYPE_SYMTAB_ADDRESS (TREE_TYPE (field));
	  p2->voffset = TREE_VIRTUAL (field)
	    ? TREE_INT_CST_LOW (DECL_VINDEX (field)) + 1 : 0;
	  p2++;
	  if (TREE_PRIVATE (field))
	    B_SET (private_bits, len2);
	  else if (TREE_PROTECTED (field))
	    B_SET (protected_bits, len2);
	  my_addrs[len2+1] = my_addrs[len2] + len3;
	  my_args[len2] = addrvec3;
	}

      fwrite (&buffer, sizeof (buffer), 1, symfile);
      fwrite (private_bits, sizeof (int), len2, symfile);
      fwrite (protected_bits, sizeof (int), len2, symfile);
      next_address += sizeof (struct type) + 2 * len2 * sizeof (int);

      for (i = 0; i < len2; i++)
	{
	  fn_fields[i].args = (struct type **)(len2 * sizeof (struct fn_field) + my_addrs[i]);
	  fn_fields[i].physname = (char *)next_address;
	  symout_strings_skip (0, IDENTIFIER_LENGTH (DECL_NAME (TREE_TYPE (field))), 0, 0);
	}

      fwrite (fn_fields, sizeof (struct fn_field), len2, symfile);
      next_address += len2 * sizeof (struct fn_field);

      for (i = 0; i < len2; i++)
	{
	  fwrite (my_args[i], sizeof (int), my_addrs[i+1]-my_addrs[i], symfile);
	  next_address += sizeof (int) * (my_addrs[i+1] - my_addrs[i]);
	}
    }
}
