...
/* write whatever format, for extended debuggers.  */

int write_symbols_plus = 0;
...
/* Nonzero means warn about implicit declarations.  Default to non-zero
   for C++. */
...
/* Nonzero means do stupid register allocation.  -noreg.
   This and `optimize' are controlled by different switches in c++,
...
/* C++: also number of features not implemented.  */
int errorcount = 0;
int warningcount = 0;
int sorrycount = 0;
...
int flag_char_charconst;

/* Nonzero used to enable ANSI brain-damaged interpretation of `const'.  */

int flag_const_is_variable;

/* Nonzero means that we cannot make optimizing assumptions about `this'.
   Blow this off, and do it right in 1.22.0.  */

int flag_this_is_variable = 1;

/* Nonzero to warn about features which are not strict C++.  Not
   yet used.  */
int pedantic_plus_plus;
...
int parse_time;
int varconst_time;
int integration_time;		/* C++: procedure integration */
int jump_time;
int cse_time;
int loop_time;
int flow_time;
...
void
announce_function (cname, decl, parms)
     tree cname, decl;
{
  if (! quiet_flag)
    {
      fprintf (stderr, "%s; ", hack_function_decl (0, cname, decl, 0));
      fflush (stderr);
    }
}

	  ...
	  if (!quiet_flag)
	  ...
	  else if (current_class_name)
	    {
	      fprintf (stderr, "In method %s:\n",
		       hack_function_decl (0, current_class_name, current_function_decl));
	    }
	  else
	    {
	      fprintf (stderr, "In function %s:\n",
		       hack_function_decl (0, NULL_TREE, current_function_decl));
	    }
     ...
     char *s;
{
  ...
  if (TREE_CODE (decl) == FUNCTION_DECL)
    {
      char *fndecl_name = hack_function_decl (0, 0, decl, 1);
      fprintf (stderr, s, fndecl_name);
    }
...
void
yylineerror (line, s, v, v2)
     int line;
     char *s;
     int v;
{
  error_with_file_and_line (input_filename, line, s, v, v2);
}
     ...
     char *name;
{
  ...
  if (rtl_dump)
    {
      register char *dumpname = (char *) alloca (dump_base_name_length + 6);
  ...
  if (combine_dump)
    {
      register char *dumpname = (char *) alloca (dump_base_name_length + 10);
      strcpy (dumpname, dump_base_name);
  ...
  if (jump2_opt_dump)
    {
      register char *dumpname = (char *) alloca (dump_base_name_length + 7);
      strcpy (dumpname, dump_base_name);
...
/* This is called from finish_union in decl.c, from shadow_tag
   (in decl.c).  It is supposed to compile the anonymous union
   so that it behaves like a union should.

   ELEMS is the list of elements in the union.  The TREE_PURPOSE
   field is the element's asmspec, and it TREE_VALUE field
   is the declaration itself.  TOP_LEVEL is nonzero if this
   declaration is not within a function.  */
void
rest_of_anon_union_compilation (decl, elems, top_level, at_end)
     tree decl, elems;
     int top_level;
     int at_end;
{
  TIMEVAR (varconst_time,
	   {
	     assemble_union (decl, elems, top_level, write_symbols, at_end);
	   });
...
/* Entry point of c++.  Decode command args, then call compile_file.
   ...
   33 if had nonfatal errors,
   32 if features were missing from the compiler, else success.  */
	...
	if (str[0] == 'm')
	...
	else if (str[0] == 'f')
	  {
	    ...
	    if (!strcmp (p, "float-store"))
	      ...
	      flag_no_asm = 1;
	    else if (!strcmp (p, "labels-ok"))
	      flag_labels_ok = 1;
	    else if (!strcmp (p, "no-labels-ok"))
	      flag_labels_ok = 0;
	    ...
	    /* C++ extensions */
	    else if (!strcmp (p, "no-strict-prototype"))
	      strict_prototype = 0;
	    else if (!strcmp (p, "no-warn-implicit"))
	      warn_implicit = 0;
	      ...
	      flag_all_virtual = 1;
	    else if (!strcmp (p, "char-charconst"))
	      flag_char_charconst = 1;
	    else if (!strcmp (p, "const-is-variable"))
	      flag_const_is_variable = 1;
	    else
...
#define __VERSION__ "[unknown]"
	  ...
	  warn_return_type = 1;
	else if (!strcmp (str, "Wno-return-type"))
	  warn_return_type = 0;
	else if (!strcmp (str, "p"))
#if 0
	  profile_flag = 1;
#else
	  fatal ("profiling not yet supported in GNU CC");
#endif
