...
#define DEFTREECODE_X(SYM, NAME, TYPE, LENGTH, ERR_NAME) TYPE,
#define DEFTREECODE_XY(SYM, NAME, TYPE, LENGTH, ERR_NAME, ERR_ASG_NAME) TYPE,
...
#define DEFTREECODE_X(SYM, NAME, TYPE, LENGTH, ERR_NAME) LENGTH,
#define DEFTREECODE_XY(SYM, NAME, TYPE, LENGTH, ERR_NAME, ASG_NAME) LENGTH,
...
#if 0
/* Return a fresh TREE_LIST for the given list.  LIST must
   be a TREE_LIST node.  */
tree copy_list (list)
     tree list;
{
  tree head, tmp;
  if (! list) return NULL_TREE;
  head = copy_node (list);
  list = TREE_CHAIN (list);
  tmp = head;
  while (list)
    {
      TREE_CHAIN (tmp) = copy_node (list);
      tmp = TREE_CHAIN (tmp);
      list = TREE_CHAIN (list);
    }
  return head;
  ...
  if (language_lvalue_valid (ref))
      ...
      case CALL_EXPR:
	{
	  ...
	  if (TREE_CODE (TREE_TYPE (ref)) == REFERENCE_TYPE)
	    return 1;
	  fndecl = TREE_OPERAND (ref, 0);
	  if (TREE_CODE (fndecl) == ADDR_EXPR)
	    {
	      fndecl = TREE_OPERAND (fndecl, 0);
	      if (TREE_CODE (fndecl) == FUNCTION_DECL
		  && TREE_RETURNS_FIRST_ARG (TREE_TYPE (fndecl)))
		return lvalue_p (TREE_VALUE (TREE_OPERAND (ref, 1)));
	    }
	}
	break;

	/* A currently unresolved scope ref.  When it is
	   needed as an lvalue, it can be converted
	   to the true reference using
	   IDENTIFIER_GLOBAL_VALUE (DECL_STATIC_NAME (...)).  */
      case SCOPE_REF:
	if (TREE_CODE (TREE_OPERAND (ref, 1)) == FIELD_DECL)
	  return 1;
	break;
      }
  return 0;
...
tree
build_type_variant (type, constp, volatilep, member_type)
     tree type, member_type;
     int constp, volatilep;
{
  ...
  for (t = m; t; t = TYPE_NEXT_VARIANT (t))
	...
	&& member_type == TYPE_DOMAIN (t))
      return t;

  /* We need a new one.  */
  current_obstack = TREE_PERMANENT (type) ? &permanent_obstack : saveable_obstack;

  t = copy_node (type);
  TREE_READONLY (t) = constp;
  TREE_VOLATILE (t) = volatilep;
  TREE_MEMBER_POINTER (t) = member_type ? 1 : 0;
  TYPE_DOMAIN (t) = member_type;
  TYPE_POINTER_TO (t) = 0;
...
/* Given two lists of types
   ...
   C++: default initializers in TREE_PURPOSE slots.  Currently
   very crude determination if two initializers are equal.  */
     ...
     tree t1, t2;
{
  ...
  if (code1 == NOP_EXPR || code1 == CONVERT_EXPR)
    if (code2 == NOP_EXPR || code2 == CONVERT_EXPR)
      return tree_init_cst_equal (TREE_OPERAND (t1, 0), TREE_OPERAND (t2, 0));
    else
      return tree_init_cst_equal (TREE_OPERAND (t1, 0), t2);
  else if (code2 == NOP_EXPR || code2 == CONVERT_EXPR)
    return tree_init_cst_equal (t1, TREE_OPERAND (t2, 0));
  ...
  switch (code1)
    ...
    case CONSTRUCTOR:
      error ("cannot handle constructors in tree_init_cst_equal");
      return 1;
    ...
    case RSHIFT_EXPR:
      return tree_init_cst_equal (TREE_OPERAND (t1, 0), TREE_OPERAND (t2, 0))
	&& tree_init_cst_equal (TREE_OPERAND (t1, 1), TREE_OPERAND (t2, 1));
    ...
    default:
      error ("Unknown _CST to tree_init_cst_equal");
      abort ();
    }
}

     ...
     tree elt_type, index_type;
{
  ...
  /* Push these needs up so that initialization takes place
     more easily.  */
  TREE_NEEDS_CONSTRUCTOR (t) = TREE_NEEDS_CONSTRUCTOR (TYPE_MAIN_VARIANT (elt_type));
  TREE_NEEDS_DESTRUCTOR (t) = TREE_NEEDS_DESTRUCTOR (TYPE_MAIN_VARIANT (elt_type));

  /* Only fill in this field if really necessary.
     @@ Note that TREE_NEEDS_DESTRUCTOR => TREE_NEEDS_CONSTRUCTOR.  */
  if (TREE_NEEDS_CONSTRUCTOR (t))
    {
      TYPE_NELTS (t) = tree_int_cst_equal (TYPE_MIN_VALUE (index_type), integer_zero_node)
