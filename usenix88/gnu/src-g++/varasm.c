     ...
     int at_end;
{
  ...
  if (DECL_RTL (decl) == 0
      ...
      else if (!top_level && !TREE_EXTERNAL (decl) && asmspec == 0)
	{
      ...
      /* C++: Make this _DECL look like an actual constant.  */
      if (TREE_LITERAL (decl))
	{
	  tree init = DECL_INITIAL (decl);
	  rtx x;

	  if (TREE_CODE (init) == INTEGER_CST)
	    x = gen_rtx (CONST_INT, DECL_MODE (init), TREE_INT_CST_LOW (init));
	  else
	    x = TREE_CST_RTL (init);
	  DECL_RTL (decl) = x;
	}
  ...
  /* Don't output anything when a tentative file-scope definition is seen.
     ...
     C++: do not lay out virtual function tables until end.  */
  if (! at_end && ((top_level && DECL_INITIAL (decl) == 0)
		   || TREE_VIRTUAL (decl)))
    return;
  ...
  /* do not write out virtual function tables that are never used.  */
  if (at_end && TREE_VIRTUAL (decl) && ! TREE_EVERUSED (decl))
    return;
...
/* Assmeble an anonymous union.  ELEMS is a list of union
   elements.  The TREE_PURPOSE field is the size to put
   out for this node, and the TREE_VALUE is the decl
   in the union.  The size for all nodes should be
   `integer_zero_node', except for the last one, which should
   be the size of the union.
   TOP_LEVEL is nonzero if this union has file scope.
   WRITE_SYMBOLS is 2 if writing dbx symbol output.
   The dbx data for a file-scope variable is written here.
   @@ This is not finished.  */
void
assemble_union (anon_union_decl, elems, top_level, write_symbols, at_end)
     register tree anon_union_decl, elems;
     int top_level;
     int write_symbols;
     int at_end;
{
  /* This is the type of the union.  It gives us the TYPE_SIZE and
     TYPE_SIZE_UNIT we will need for outputting this union.  */
  ...
  if (TREE_READONLY (anon_union_decl)
      && ! TREE_VOLATILE (anon_union_decl))
    fprintf (asm_out_file, "%s\n", TEXT_SECTION_ASM_OP);
  else
    fprintf (asm_out_file, "%s\n", DATA_SECTION_ASM_OP);

  for (i = 0; DECL_ALIGN (anon_union_decl) >= BITS_PER_UNIT << (i + 1); i++);

  ASM_OUTPUT_ALIGN (asm_out_file, i);

  for (; elems; elems = TREE_CHAIN (elems))
    {
      tree asmspec = TREE_PURPOSE (elems);
      tree decl = TREE_VALUE (elems);
      register char *name = IDENTIFIER_POINTER (DECL_NAME (decl));
	  ...
	  if (!top_level && !TREE_EXTERNAL (decl) && asmspec == 0)
	    {
	      ...
	      /* See comment above about rtx obstack.  */
	      name = obstack_copy0 (current_obstack, label, strlen (label));
	    }
	...
	continue;
	 ...
	 (which is what it must have been) as an `extern' reference.  */
      ...
      if (DECL_SIZE (decl) == 0)
	{
	  error_with_decl (decl, "storage size of static anonymous union element `%s' isn't known");
	  continue;
	}
	...
	continue;
      ...
      if (TREE_PUBLIC (anon_union_decl))
	ASM_GLOBALIZE_LABEL (asm_out_file, name);
      ASM_OUTPUT_LABEL (asm_out_file, name);
    }
  /* N.B.: No initialization of unions.  */
  i = int_size_in_bytes (TREE_TYPE (anon_union_decl));
  if (i > 0)
    ASM_OUTPUT_SKIP (asm_out_file, i);
}

...

/* C++: Walk a tree, outputing constant labels where needed.  */
void
make_rtx_constant_now (t)
     register tree t;
{
  switch (TREE_CODE (t))
    {
    ...
    case ADDR_EXPR:
      {
	register tree constant = TREE_OPERAND (t, 0);
    ...
    case POSTINCREMENT_EXPR:
      make_rtx_constant_now (TREE_OPERAND (t, 0));
      break;
    ...
    case NE_EXPR:
      make_rtx_constant_now (TREE_OPERAND (t, 0));
      make_rtx_constant_now (TREE_OPERAND (t, 1));
      break;

    case COND_EXPR:
      make_rtx_constant_now (TREE_OPERAND (t, 0));
      make_rtx_constant_now (TREE_OPERAND (t, 1));
      make_rtx_constant_now (TREE_OPERAND (t, 2));
      break;
    ...
    case CALL_EXPR:
      ...
      while (t)
	{
	  make_rtx_constant_now (TREE_VALUE (t));
	  t = TREE_CHAIN (t);
	}
      break;

    case CONSTRUCTOR:
      /* Make the list of constructor elements all have constant rtxs.  */
      t = TREE_OPERAND (t, 1);
      while (t)
	{
	  make_rtx_constant_now (TREE_VALUE (t));
    ...
    case NEW_EXPR:
    case DELETE_EXPR:
      sorry ("new or delete expression in default parameter list");
      return;
