...
/*  */
    ...
    if (filter)
	mhl_draft (out, digest, drft, filter, dashflg);
    else
...
static	mhl_draft  (out, digest, file, filter, dashflg)
int     out,
	dashflg;
...
static	copy_draft (out, digest, file)
int     out;
register char   *digest,
		*file;
{
    ...
    if (digest) {
	(void) sprintf (buffer, "End of %s Digest\n", digest);
	i = strlen (buffer);
